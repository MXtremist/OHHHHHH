...
int myremove(FILE *file, Inode *inode, Inode *fatherinode, SuperBlock *superBlock, int fatherInodeOffset, int inodeOffset)
{
    //free inode
    ...
    for (int index = 0; index < inode->blockCount; index++)
    {
        ...
        if (index < bound0) //level 1
            insideblockOffset = (inode->pointer[index] - superBlock->blocks) * SECTOR_SIZE;
        else if (index < bound1) //level 2
        {
            fseek(file, inode->singlyPointer * SECTOR_SIZE, SEEK_SET);
            fread((void *)singlyPointerBuffer, sizeof(uint8_t), superBlock->blockSize, file);
            insideblockOffset = (singlyPointerBuffer[index - bound0] - superBlock->blocks) * SECTOR_SIZE;
        }
        else
            return -1;
    ...
    {
        ret = readBlock(file, superBlock, fatherinode, i_dir, buffer);
        if (ret == -1)
            return -1;
        dirEntry = (DirEntry *)buffer;
    ...
    ret = writeBlock(file, superBlock, fatherinode, i_dir, buffer);
    if (ret == -1)
        return -1;
    fseek(file, 0, SEEK_SET);
    fwrite((void *)superBlock, sizeof(SuperBlock), 1, file);
...
int rmdir(const char *driver, const char *destDirPath)
{
    ...
    ret = readInode(file, &superBlock, &inode, &inodeOffset, destDirPath);
    if (ret == -1)
    {
    ...
    while (getDirEntry(file, &superBlock, &inode, dirIndex, &dirEntry) == 0)
    {
        ...
        if (ret == -1)
        {
            printf("Failed to rm %s in dir %s.\n", dirEntry.name, destDirPath);
            fclose(file);
            return -1;
        }
        printf("rm %s in dir %s.\n", dirEntry.name, destDirPath);
    }
...
int rm(const char *driver, const char *destFilePath, int state)
{
    // TODO
    FILE *file = NULL;
    char tmp = 0;
    ...
    *((char *)destFilePath + size + 1) = tmp;
    ...
    if (state == 0)
        printf("rm %s ", destFilePath);
    ret = myremove(file, &inode, &fatherInode, &superBlock, fatherInodeOffset, inodeOffset);
    fclose(file);
    if (ret == -1)
    {
